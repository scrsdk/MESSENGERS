...
export const ChatCard = ({
  ...
  useEffect(() => {
    ...
    if (
      lastMsgDataProp?.roomID &&
      lastMsgDataProp?.roomID !== selectedRoom?._id
    )
      return;
    ...
    if (selectedRoom?.messages?.length) {
      for (let i = -1; i <= selectedRoom.messages.length; i--) {
        if (!selectedRoom.messages.at(i)?.hideFor?.includes(myID)) {
          // if we didn't removed the message for ourselves.
    ...
    >
      ...
      <div className="flex flex-col w-full ch:w-full gap-1 text-darkGray text-[14px]">
        <div className="flex items-center justify-between">
          <p className="text-white font-bold text-[16px] font-segoeBold line-clamp-1">
