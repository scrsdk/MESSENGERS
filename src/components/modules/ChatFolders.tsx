...
const ChatFolders = ({ name, count, isActive, onClick }: Props) => {
  return (
    ...
    >
      ...
      {count ? (
        <span
          className={`px-[5px] text-[13px] h-[18px] min-w-[18px] w-min flex items-center justify-center text-center ml-1 pb-px ${
      ...
      {isActive && (
        <span className="absolute inset-x-0 -bottom-1 bg-lightBlue rounded-t-md w-full h-1"></span>
      )}
    </div>
  );
};
